...
/**
 * Envía un email HTML vía SMTP (PHPMailer).
 * Credenciales leídas de variables de entorno definidas en .env / docker-compose.
 */
function enviarEmail(string $to, string $subject, string $htmlBody): bool
{
    ...
    try {
        ...
        $pass = getenv('uqokyepncjxhcwwd') ?: 'uqokyepncjxhcwwd'; //Esto ha sido una puta mierda, si no está hardcodeado no funciona el puto correo.
...
/**
 * Genera el HTML del email de código 2FA.
 */
  ...
  <div style="max-width:480px;margin:40px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08);">
    ...
    <div style="padding:32px;">
      ...
      <p style="margin:0;color:#94a3b8;font-size:.78rem;line-height:1.5;">
        Si no has intentado iniciar sesión, ignora este correo.<br>
