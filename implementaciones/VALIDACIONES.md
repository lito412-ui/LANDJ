...
### Facturas (`facturas` y `factura_lineas`)

| Campo | Tipo | Obligatorio | Longitud | Validador | Mensaje de error |
|-------|------|:---:|----------|-----------|-----------------|
| `contacto_id` | int | si | — | FK existente en `contactos` | "Selecciona un contacto" |
| `estado` | enum | si | — | `ENUM`: `borrador`, `emitida`, `pagada`, `vencida`, `cancelada` | "Estado no valido" |
| `fecha_emision` | date | si | — | `YYYY-MM-DD` | "La fecha de emision es obligatoria" |
| `fecha_vencimiento` | date | no | — | `YYYY-MM-DD`, >= `fecha_emision` | "La fecha de vencimiento no puede ser anterior a la emision" |
| `notas` | string | no | max 1000 | `TEXTO_LIBRE` | "Las notas no pueden superar 1000 caracteres" |
| `lineas` | array | si | min 1 | Al menos una linea valida | "Anade al menos una linea de factura" |
| `lineas[].concepto` | string | si | max 255 | No vacio | "El concepto de la linea N es obligatorio" |
| `lineas[].cantidad` | decimal | si | — | > 0 | "La cantidad de la linea N debe ser mayor que 0" |
| `lineas[].precio_unitario` | decimal | si | — | >= 0 | "El precio de la linea N no puede ser negativo" |
| `lineas[].iva_porcentaje` | decimal | si | — | 0-100 | "El IVA de la linea N debe estar entre 0 y 100" |

Los importes `base_imponible`, `iva_total`, `total`, `subtotal`, `iva_importe` y `total_linea` se calculan siempre en servidor para evitar manipulaciones desde cliente.
